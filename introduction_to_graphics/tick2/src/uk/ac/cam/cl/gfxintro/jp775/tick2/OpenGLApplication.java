package uk.ac.cam.cl.gfxintro.jp775.tick2;
...
public class OpenGLApplication {
      ...
      //generate and upload vertex data
        ...
        for (int row = 0; row < heightmap_height_px; row++) {
            for (int col = 0; col < heightmap_width_px; col++) {
                int index = (row * heightmap_width_px + col) * 3;
                vertPositions[index] = start_x + col * delta_x;
                vertPositions[index + 1] = heightmap[row][col];
                vertPositions[index + 2] = start_z + row * delta_z;
            }
        }
    ...
    public float[] initializeVertexNormals( float[][] heightmap ) {
        ...
        for (int row = 0; row < heightmap_height_px; row++) {
            for (int col = 0; col < heightmap_width_px; col++) {
                ...
                //if we're not on an edge, do the proper cross product
                if (row * col != 0 && row != heightmap_height_px - 1 && col != heightmap_width_px - 1) {
                    ...
                    v = Tz.cross(Tx).normalize();
                }

                vertNormals[index] = v.x;
                vertNormals[index + 1] = v.y;
                vertNormals[index + 2] = v.z;
            }
        }
    ...
    public void loadDataOntoGPU( float[] vertPositions, int[] indices, float[] vertNormals ) {
        ...
        // Construct the normal buffer in CPU memory
        ...
        glBindBuffer(GL_ARRAY_BUFFER, glGenBuffers()); // Bring buffer object into existence on GPU
        glBufferData(GL_ARRAY_BUFFER, normal_buffer, GL_STATIC_DRAW); // Load the GPU buffer object with data

        int normalLocation = glGetAttribLocation(shaders_handle, "normal");
        // If vertex attribute does not exist
        if (normalLocation == -1) throw new RuntimeException( "'normal' variable not found in the shader file");

        // Specifies where the data for "normal" variable can be accessed
        glVertexAttribPointer(normalLocation, 3, GL_FLOAT, false, 0, 0);

        // Enable that normal attribute variable
